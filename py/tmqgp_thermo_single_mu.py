...
for T, LT in zip(Trange, LTs):
    ...
    for keys, _ in zip([keys_QQ, keys_QA, keys_QG], ['QQ', 'QA', 'QG']):
        for k in keys:
...
for T, LT in zip(Trange, LTs):
    ...
    for keys, _ in zip([keys_AQ, keys_AA, keys_AG], ['AQ', 'AA', 'AG']):
        for k in keys:
            ds = df['V'][k]['0'].attrs['ds']
            da = df['V'][k]['0'].attrs['da']
            Fa = df['V'][k]['0'].attrs['Fa']
...
for i, T, LTs in zip(range(len(Trange)), Trange, LTs_Q):
    ...
    for key, func, p2 in zip(['QQ', 'QG', 'QA'], 
                [tm.sigma_ff_onshell, tm.sigma_fb_onshell, tm.sigma_ff_onshell], 
                [pQs[i], pGs[i], pAs[i]]):
        LT = LTs[key]
...
for i, T, LTs in zip(range(len(Trange)), Trange, LTs_A):
    ...
    ReLogSs_A = []
    # iterating over types of diagrams -- QQ and QG
    for key, func, p2 in zip(['AA', 'AG', 'AQ'], 
                [tm.sigma_ff_onshell, tm.sigma_fb_onshell, tm.sigma_ff_onshell], 
                [pAs[i], pGs[i], pQs[i]]):
        LT = LTs[key]
