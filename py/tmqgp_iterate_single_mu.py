...
# 
    ...
    if n_iter == 0:
        for k, ch_l in (list(channels_QQ.channels.items()) + list(channels_QG.channels.items()) 
                    ...
                    list(channels_AQ.channels.items()) + list(channels_AG.channels.items()) +
                    list(channels_GA.channels.items())):
    ...
    funcs = [tm.sigma_ff_onshell, tm.sigma_ff_onshell, tm.sigma_fb_onshell, 
             tm.sigma_ff_onshell, tm.sigma_ff_onshell, tm.sigma_fb_onshell, 
             tm.sigma_bf_onshell, tm.sigma_bf_onshell, tm.sigma_bb_onshell]

    chg_list = [channels_QQ, channels_QA, channels_QG,
                channels_AQ, channels_AA, channels_AG,
                channels_GQ, channels_GA, channels_GG]
    ...
    ImS_Q = IMAGs['QQ'] + IMAGs['QG'] + IMAGs['QA']
    ReS_Q = REALs['QQ'] + REALs['QG'] + REALs['QA']
...
##########
    ImS_A = IMAGs['AA'] + IMAGs['AG'] + IMAGs['AQ']
    ReS_A = REALs['AA'] + REALs['AG'] + REALs['AQ']
...
####    
    ImS_G = IMAGs['GG'] + IMAGs['GQ'] + IMAGs['GA']
    ReS_G = REALs['GG'] + REALs['GQ'] + REALs['GA']
...
#         print(delta)  
    ...
    pts += [[quark_run, aquark_run, gluon_run]]
    ...
    print('T = %.3f iteration %i: delta = %f, sum Q = %f, sum G = %f sum A = %f'%(T, n_iter,
    delta, sumQ, sumG, sumA))
