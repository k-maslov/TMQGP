...
double x_integral_cm_onshell_allom(double omp, double om, double p, 
    ...
    funct i_func_x = [&](double x) -> double {
        return x_cm_onshell_integrand_allom(x, omp, om ,p, k, iImT, iImG, eps1, eps2, debug, l);
    };
...
int l){
    ...
    // printf("Hello!!!\n");
    funct i_func_k = [&](double k) -> double {
        return x_integral_cm_onshell_allom(omp, om, p, k, iImT, iImG, eps1, eps2, 0, l) *  iImG(k, omp);
    };
    integ_k.integrate(&i_func_k, 0, 5, res, err);
    return res;
}
...
double sigma_ff_onshell_allom(double om, double p, double T, 
    ...
    gsl_set_error_handler_off();
    funct i_func_e = [&](double omp) -> double {
        double res = k_integral_onshell_allom(omp, om, p, iImT, iImG, eps1, eps2, l) *
         (n_f(omp, T) + n_b(omp + om, T));
        return res;
    };
...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%% Test disabling the spacelike integration %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    double res = k*k * iImT(sqrt(k2), 
                ((om + omp > 0) - (om + omp < 0)) * Ecm)  / 4 / M_PI/ M_PI *
         (n_f(omp, T) + n_b(omp + om, T)) *  iImG(k, omp);
